...
def piecewise_index(relative_position, alpha, beta, gamma, dtype):
    ...
    idx: torch.Tensor, dtype: long
        ...
        `idx` is a long tensor,
         whose shape is (L, L) and each element is in [-beta, beta].
    """
    ...
    y_out = (
        torch.sign(rp_out) *  # noqa
        (
            alpha +  # noqa
            torch.log(rp_abs_out / alpha) / math.log(gamma / alpha) *  # noqa
...
def get_absolute_positions(height, width, dtype, device):
    ...
    '''
    rows = torch.arange(height, dtype=dtype,
                        device=device).view(height, 1).repeat(1, width)
    cols = torch.arange(width, dtype=dtype,
                        device=device).view(1, width).repeat(height, 1)
...
def quantize_values(values):
    ...
    uq.numel(): long
        The number of the quantization integers,
         namely `res` is in [0, uq.numel()).
    """
...
class METHOD:
    """define iRPE method IDs We divide the implementation of CROSS into
    CROSS_ROWS and CROSS_COLS."""
    EUCLIDEAN = 0
    QUANT = 1
    PRODUCT = 3
    CROSS = 4
...
@torch.no_grad()
def get_bucket_ids_2d_without_skip(method,
                                   height,
                                   width,
                                   alpha,
                                   beta,
    ...
    if value is None or value[-2] < height or value[-1] < width:
        ...
        if method != METHOD.PRODUCT:
            bucket_ids += beta_int
        bucket_ids = bucket_ids.view(max_height, max_width, max_height,
                                     max_width)
...
class iRPE(nn.Module):
    ...
    def __init__(self,
        ...
        assert method is not None,\
            'method should be a METHOD ID rather than None'
        self.method = method
    ...
    def _get_rp_bucket(self, x, height=None, width=None):
        """Get relative position encoding buckets IDs
         corresponding the input shape
        Parameters
        ----------
        x: torch.Tensor
            Input Tensor whose shape is (B, H, L, head_dim),
        ...
        if self.mode == 'contextual' and self.transposed:
            offset = torch.arange(0,
                                  ...
                                  device=rp_bucket.device).view(-1, 1)
...
def get_single_rpe_config(ratio=1.9,
    ...
    config.num_buckets = get_num_buckets(method, config.alpha, config.beta,
                                         config.gamma)
...
def build_rpe(config, head_dim, num_heads):
    ...
    return [
        _build_single_rpe(rpe, transposed)
