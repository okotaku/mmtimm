# --------------------------------------------------------
# Model from official source: https://github.com/Andrew-Qibin/VisionPermutator.
# --------------------------------------------------------
...
class WeightedPermuteMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        S = self.dim // H
        segment_dim = C // S
        h = x.reshape(B, H, W, segment_dim,
                      S).permute(0, 3, 2, 1,
                                 4).reshape(B, segment_dim, W, H * S)
        h = self.mlp_h(h).reshape(B, segment_dim, W, H,
                                  S).permute(0, 3, 2, 1,
                                             4).reshape(B, H, W, C)

        w = x.reshape(B, H, W, segment_dim,
                      S).permute(0, 1, 3, 2,
                                 4).reshape(B, H, segment_dim, W * S)
        w = self.mlp_w(w).reshape(B, H, segment_dim, W,
                                  S).permute(0, 1, 3, 2,
                                             4).reshape(B, H, W, C)
...
def basic_blocks(dim,
    ...
    for block_idx in range(layers[index]):
        ...
        block_dpr = drop_path_rate * (
                block_idx + sum(layers[:index])  # noqa
        ) / (sum(layers) - 1)
        # yapf:enable
...
def _create_vip(variant, pretrained, **kwargs):
    return build_model_with_cfg(VisionPermutator,
                                variant,
                                pretrained,
...
def vip_s7(pretrained=False, **kwargs):
    ...
    model = _create_vip('vip_s7',
                        ...
                        mlp_fn=WeightedPermuteMLP,
                        **kwargs)
    model.default_cfg = default_cfgs['vip_s7']
    return model
...
def vip_m7(pretrained=False, **kwargs):
    ...
    model = _create_vip('vip_m7',
                        ...
                        mlp_fn=WeightedPermuteMLP,
                        **kwargs)
    model.default_cfg = default_cfgs['vip_m7']
    return model
...
def vip_l7(pretrained=False, **kwargs):
    ...
    model = _create_vip('vip_l7',
                        ...
                        mlp_fn=WeightedPermuteMLP,
                        **kwargs)
    model.default_cfg = default_cfgs['vip_l7']
    return model
