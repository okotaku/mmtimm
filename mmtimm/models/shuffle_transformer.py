# --------------------------------------------------------
# Model from official source: https://github.com/mulinmeng/Shuffle-Transformer
# --------------------------------------------------------
import torch
...
class StageModule(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        assert layers % 2 == 0,\
            'Stage layers need to be divisible by 2' \
            ' for regular and shifted block.'
...
class ShuffleTransformer(nn.Module):
    def __init__(self,
        ...
        self.has_pos_embed = has_pos_embed  # todo: make pos emb
        dims = [i * 32 for i in num_heads]
        self._set_feature_info(features_only, out_indices, dims)

        self.to_token = PatchEmbedding(inter_channel=token_dim,
...
@register_model
def shuffle_vit_base_patch4_window7_224(pretrained=False, *args, **kwargs):
    return _shuffle_transformer('shuffle_vit_base_patch4_window7_224',
                                pretrained=pretrained,
                                patch_size=4,
                                embed_dim=128,
                                layers=[2, 2, 18, 2],
...
@register_model
def shuffle_vit_small_patch4_window7_224(pretrained=False, *args, **kwargs):
    return _shuffle_transformer('shuffle_vit_small_patch4_window7_224',
                                pretrained=pretrained,
                                patch_size=4,
                                embed_dim=96,
                                layers=[2, 2, 18, 2],
...
@register_model
def shuffle_vit_tiny_patch4_window7_224(pretrained=False, *args, **kwargs):
    return _shuffle_transformer('shuffle_vit_tiny_patch4_window7_224',
                                pretrained=pretrained,
                                patch_size=4,
                                embed_dim=96,
                                layers=[2, 2, 6, 2],
